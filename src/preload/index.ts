...
const api: ApiaryApi = {
  ...
  tabDragStart: (key) => ipcRenderer.send(CHANNELS.tabDragStart, key),
  tabDragEnd: () => ipcRenderer.send(CHANNELS.tabDragEnd),
  tabDragCurrent: () => ipcRenderer.invoke(CHANNELS.tabDragCurrent),
  tabDetach: (key, at) => ipcRenderer.invoke(CHANNELS.tabDetach, key, at),
  tabClaim: (key) => ipcRenderer.invoke(CHANNELS.tabClaim, key),
