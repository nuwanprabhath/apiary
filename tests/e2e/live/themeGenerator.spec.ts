 ...
 */
// eslint-disable-next-line playwright/no-skipped-test -- opt-in: spends real tokens generating themes with the real `claude`, so it must not run by default
...
for (const [slug, request, refracts] of [['liquid-glass', 'Mac OS like liquid glass', true], ['frosted', 'Frosted glass', false]] as const) {
  test(`real claude answers "${request}" with a glass theme`, async () => {
    ...
    // `refracts` is fixed per test case above, not a runtime branch; when it's false there is
    // nothing to assert about refraction, so the assertion is made a no-op rather than skipped
    // inside an `if`, which playwright/no-conditional-expect disallows.
    expect(refracts ? result.spec.material.refraction : 1).toBeGreaterThan(0)
    await h.page.evaluate((spec) => window.apiary.themeSave(spec.name, spec).then((s) => window.apiary.themeApply(s.id)), result.spec)
    await h.page.getByText('Fix CSV export bug').first().click().catch(() => {})
    // eslint-disable-next-line playwright/no-wait-for-timeout -- lets the newly applied theme's effects settle before the screenshot below, which is for a human to eyeball
    await h.page.waitForTimeout(1500)
